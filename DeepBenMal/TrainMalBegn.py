...
if __name__ == '__main__':
    ...
    if opt.task == "task1":
        ...
        test_iter = MBDataIterResisClassify(data_file= opt.valid_path + '/val_%d.npy' % opt.num_valid, phase='test', crop_size=opt.crop_size,
                                    crop_depth=opt.sample_duration, sample_size=opt.sample_size)
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        if opt.task == "task1":
            ...
            train_iter = MBDataIterResisClassify(data_file=opt.valid_path +'/train_%d.npy' % opt.num_valid, phase='train',
                                         crop_size=opt.crop_size, crop_depth=opt.sample_duration, sample_size=opt.sample_size, sample_phase=None)
        train_loader = DataLoader(
            train_iter,
            batch_size=opt.batch_size,
