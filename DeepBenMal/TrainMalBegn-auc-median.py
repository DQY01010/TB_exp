...
from DataIter import MBDataIter2
...
def test(model, data_loader, loss, epoch, lr, max_acc, max_auc, acc_max, auc_max, max_acc_auc):
    test_acc = []
    loss_lst = []
...
if __name__ == '__main__':
    ...
    if opt.tf:
        ...
        load_parameters = torch.load("pretrained_model/resnet-18-kinetics.pth")['state_dict']
        # model.load_state_dict(load_parameters)
        for key, value in model.state_dict().items():
            part_key = key
            if part_key in load_parameters.keys():
                model.state_dict()[key] = load_parameters[part_key]
    
    # import pdb;pdb.set_trace()
    
    # construct training data iterator
    train_iter = MBDataIter2(
        data_file='./valid_5/no_resample/train_no_sample%d.npy'%opt.num_valid,
        phase='train',
    ...
    test_iter = MBDataIter2(
        data_file='./valid_5/no_resample/val_no_sample%d.npy'%opt.num_valid,
        phase='test',
        crop_size=opt.sample_size,
        crop_depth=opt.sample_duration)
    ...
    for epoch in range(opt.start_epoch,opt.epochs):
        ...
        max_acc,max_auc,acc_max,auc_max,max_acc_auc,test_loss,isave,pred_target_dict,isave_acc_lst, isave_auc_lst, isave_acc_auc_lst = test(model, test_loader, loss, epoch,opt.lr, max_acc,max_auc,acc_max,auc_max,max_acc_auc)
        ...
        train_iter = MBDataIter2(
            data_file='./valid_5/resample_mean/train_%d.npy'%opt.num_valid,
            phase='train',
            crop_size=opt.sample_size,
            crop_depth=opt.sample_duration,
            aug=opt.aug,
